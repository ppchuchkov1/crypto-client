import fttImage from "../../assets/slot/ftt-bonus.png";
import useSlotGame from "../../zustang/useSlotGame";
import Modal from "../../UI/Modal";

const SlotFreeSpinsStartModal = () => {
  const { showFreeSpinsMessage, setShowFreeSpinsMessage } = useSlotGame();

  return (
    <Modal
      isOpen={showFreeSpinsMessage}
      onClose={() => setShowFreeSpinsMessage(false)}
      className="bg-white border border-neutral-200 shadow-2xl max-w-[95vw] sm:max-w-[500px] w-full flex flex-col items-center gap-6 animate-fade-in rounded-2xl p-4 sm:p-7"
    >
      <div className="flex flex-col items-center justify-center gap-4 w-full">
        <img
          src={fttImage}
          className="w-32 h-32 sm:w-40 sm:h-40 object-contain mb-2"
          alt="SBF"
        />
        <span className="text-2xl sm:text-4xl font-bold text-black text-center tracking-tight leading-tight">
          Free Spins
        </span>
        <span className="text-base sm:text-lg text-neutral-700 text-center max-w-[95vw] sm:max-w-[90%]">
          Congratulations! You just unlocked bonus spins.
          <br />
          Use your free spins for a chance to win even bigger prizes.
        </span>
      </div>
      <button
        onClick={() => setShowFreeSpinsMessage(false)}
        className="mt-2 px-6 py-2 sm:px-8 sm:py-3 rounded-xl bg-black text-white font-bold shadow transition-all hover:bg-neutral-800 hover:scale-105 duration-150 text-base sm:text-lg w-full sm:w-auto"
      >
        Start Spinning
      </button>
    </Modal>
  );
};

export default SlotFreeSpinsStartModal;
